...
def start_iperf(net):
    ...
    server1 = h2.popen("iperf -s -w 16m")
...
def get_timing(net, h1, h2):
    timing = []
    for i in range(3):
        fetch = "curl -o index.html -s -w \%{time_total} 10.0.0.1/http/index.html"
...
def tcp():
    ...
    #Popen("pgrep -f index.html | xargs kill -9", shell=True).wait()
